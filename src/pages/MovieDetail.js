  ...
  const image = movie.poster_path ? `https://image.tmdb.org/t/p/w500/${movie.poster_path}` : backupImg;
    ...
    useEffect (()=>{
      async function fetchMovie() {
        const response = await fetch(`https://api.themoviedb.org/3/movie/${params.id}?api_key=9ef14e1968b3e0c6dc2c1325de5b3aad`);
            ...
            <p className="my-4">{movie.overview}</p>
             ...
             {movie.genres ? ( 
                 ...
                 { movie.genres.map((genre)=> (
                  <span className="mr-2 border border-gray-800 rounded dark:border-gray-600 p-2" key={genre.id}>{genre.name}</span>
                )) }       
                </p>
                ) : ""}
                ...
                <div className="flex items-center">
                    ...
                    <p className="mr-2 text-gray-900 dark:text-white">{movie.vote_average }</p>
